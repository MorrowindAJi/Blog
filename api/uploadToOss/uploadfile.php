...
class UploadFile{
	 ...
	 * 允许上传的文件类型
	 */
	private $allow_type=array('gif','jpg','jpeg','bmp','png','swf','tbi','mp4','avi','rmvb');
	/**
	 * 允许的最大文件大小，单位为KB
	 */
	...
	public function http_postdata($url,$postdata) {
	    $opts = array(
	        'http' => array(
	            'method'  => 'POST',
	...
	function http_post_param_data($url, $param) {
	    //初始化
	    $curl = curl_init();
	    //设置抓取的url
	 ...
	 * @return bool
	 */
	public function upfile_1($field,$oss = false){
...
//      }
		...
		$key = $this->default_dir.$this->file_name;
        $qiniu = qiniu::getToken($key);
        $post_data['key'] = $key;
        $post_data['token'] = $qiniu['token'];
        if (version_compare("5.5", PHP_VERSION, ">")) {
            $post_data['file'] =  '@'.$this->upload_file['tmp_name'];
        }else{
            $post_data['file'] =  new CURLFile($this->upload_file['tmp_name']);//php5.5以后才支持
        }
//      $reuslt = http_post_param_data('http://upload-z2.qiniu.com',$post_data);
        $reuslt = $this->http_post_param_data('http://upload-z2.qiniu.com',$post_data);
        if($reuslt){
            return true;
        }else{
            return false;
...
//        }
        ...
        //检查是否为有效图片
        if((!$image_info = @getimagesize($this->upload_file['tmp_name'])) && ($this->ext != 'mp4') && ($this->ext != 'mp3')&& ($this->ext != 'wmv')) {
            $error = Language::get('upload_image_is_not_image');
            $this->setError($error);
            return false;
        }
...
//      $this->setErrorFileName($this->upload_file['tmp_name']);
	 ...
	 * @return bool
	 */
	public function upfile_video($field,$oss = false){
